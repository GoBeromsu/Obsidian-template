...
- `me.md` (vault root) — single context worksheet with English `##` headings and blank answer areas; agent-authored scaffold, not a human-stamped Templater file. It is the one deliberate root-level note exception; do not move it back into `00. Inbox`, rename its headings, or add a second `me` template.
