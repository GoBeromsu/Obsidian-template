...
Do not add, rename, or renumber roots without recording the decision in `90. Settings/01 Guideline/` (both language files). Root `Home.md` is the adopter's first table of contents (Homepage plugin opens it on startup via `.obsidian/plugins/homepage/data.json`); its `> 역할:` lines are prompts, and its lists are meant to be replaced. `90. Settings/03 Home/` ships one example MOC copied from the instructor vault (`2024 Home.md`, Why/How/What structure, links unresolved by design) and `04 Index/` ships empty; `05 Bases/` ships `Video.base` and `Books.base` as worked examples, and `85. Raw/02 Videos/` ships two real public-metadata video notes (Brian's Brain Trinity, Seoul Metaweek 2026 talk; Nick Milo, The 3-File AI System) so `Video.base` renders cards grouped by author out of the box. The second note was produced by the `.agents/skills/video` skill workflow (`scripts/fetch_youtube_meta.py` + `manual/video.template.md`), which is the supported way to add a YouTube note from a URL; `title`, `author`, and `description` must stay double-quoted or the frontmatter fails to parse and the note disappears from every Base. Both Bases exclude `90. Settings/02 Templates` so template files never appear as results.
