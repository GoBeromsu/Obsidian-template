...
- Preserve existing YAML provenance and defer metadata field names and values to the adopter's policy. Human-facing templates in this folder already stamp `created_by: user` and `authorship: user`; do not add competing metadata requirements on top of them. New agent-authored notes use the session-required `created_by` agent identifier and `authorship: agent`; report conflicts instead of inventing a competing schema.
