...
Placement SSOT is the bilingual Placement Guide under `01 Guideline/`. Templates live under `02 Templates/` (`auto/` periodic, `manual/` on-demand). `03 Home/` ships `2024 Home.md`, an example MOC from the instructor vault whose links are intentionally unresolved; `04 Index/` ships empty. `05 Bases/` ships two standalone example Bases (`Video.base`, `Books.base`) that read properties stamped by `02 Templates/manual/video.template.md` and `book.template.md`; periodic views are embedded in templates. Drawings live under `07 Excalidraw/`. `.obsidian/` is a sibling of this folder at vault root, not inside it.
